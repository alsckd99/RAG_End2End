...
from peft.utils import PeftType, _get_batch_size
...
class GenerativeQAModule(BaseTransformer):
    ...
    def __init__(self, hparams, **kwargs):
        ...
        model = RagTokenForGeneration.from_pretrained_question_encoder_generator("facebook/dpr-question_encoder-single-nq-base", "meta-llama/Llama-2-7b-chat-hf")
    ...
    def validation_step(self, batch, batch_idx) -> Dict:
      ...
      for key, value in metrics.items():
        if key not in ['preds', 'target']:
          self.log(f'val_{key}', value, on_step=False, on_epoch=True, prog_bar=True, logger=True,sync_dist=True)
      return metrics

    def on_validation_epoch_end(self):
        self.step_count += 1
        val_loss = self.trainer.logged_metrics['val_loss'].item() if isinstance(self.trainer.logged_metrics['val_loss'], torch.Tensor) else self.trainer.logged_metrics['val_loss']
        val_gen_time = self.trainer.logged_metrics['val_gen_time'].item() if isinstance(self.trainer.logged_metrics['val_gen_time'], torch.Tensor) else self.trainer.logged_metrics['val_gen_time']
        val_gen_len = self.trainer.logged_metrics['val_gen_len'].item() if isinstance(self.trainer.logged_metrics['val_gen_len'], torch.Tensor) else self.trainer.logged_metrics['val_gen_len']
        val_em = self.trainer.logged_metrics['val_em'].item() if isinstance(self.trainer.logged_metrics['val_em'], torch.Tensor) else self.trainer.logged_metrics['val_em']
        val_f1 = self.trainer.logged_metrics['val_f1'].item() if isinstance(self.trainer.logged_metrics['val_f1'], torch.Tensor) else self.trainer.logged_metrics['val_f1']
        
        latest_metrics = {
        'step_count': self.step_count, 
        'val_loss': val_loss,
        'val_gen_time': val_gen_time,
        'val_gen_len': val_gen_len,
        'val_em': val_em,
        'val_f1': val_f1
        }
        self.save_metrics(latest_metrics, "val")
    ...
    def calc_generative_metrics(self, preds, target) -> Dict:
      em_result = calculate_exact_match(preds, target)
      f1_result = calculate_f1_score(preds, target)
      return {"em": em_result["em"], "f1": f1_result["f1"]}
    ...
    def on_test_epoch_end(self):
        self.step_count += 1
        test_loss = self.trainer.logged_metrics['test_loss'].item() if isinstance(self.trainer.logged_metrics['test_loss'], torch.Tensor) else self.trainer.logged_metrics['test_loss']
        test_gen_time = self.trainer.logged_metrics['test_gen_time'].item() if isinstance(self.trainer.logged_metrics['test_gen_time'], torch.Tensor) else self.trainer.logged_metrics['test_gen_time']
        test_gen_len = self.trainer.logged_metrics['test_gen_len'].item() if isinstance(self.trainer.logged_metrics['test_gen_len'], torch.Tensor) else self.trainer.logged_metrics['test_gen_len']
        test_em = self.trainer.logged_metrics['test_em'].item() if isinstance(self.trainer.logged_metrics['test_em'], torch.Tensor) else self.trainer.logged_metrics['test_em']
        test_f1 = self.trainer.logged_metrics['test_f1'].item() if isinstance(self.trainer.logged_metrics['test_f1'], torch.Tensor) else self.trainer.logged_metrics['test_f1']
        latest_metrics = {
          'step_count': self.step_count,
          'test_loss': test_loss,
          'test_gen_time': test_gen_time,
          'test_gen_len': test_gen_len,
          'test_em': test_em,
          'test_f1': test_f1
        }
        self.save_metrics(latest_metrics, "test")
...
def main(args=None, model=None) -> GenerativeQAModule:
    ...
    early_stopping_callback= EarlyStopping(
        monitor="val_"+model.val_metric,  # does this need avg?
        mode="min" if "loss" in model.val_metric else "max",
    ...
    trainer = pl.Trainer(
      max_epochs=3,
      callbacks=[logging_callback, checkpoint_callback,early_stopping_callback],
    ...
    trainer.fit(model)
    #trainer.fit(model,ckpt_path=args.output_dir+"/last_Q-P_full.ckpt")
    #trainer.fit(model,ckpt_path=args.output_dir+"/epoch=0-step=48.ckpt")
